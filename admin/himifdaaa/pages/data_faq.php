...
<div class="col-md-12" style="padding:0px">
    <ol class="breadcrumb" style="margin:0;border-radius:0;">
      <li class="active"><a href="index.php?halaman=index">Dashboard</a> / FAQ</li>
    </ol>
</div>
      ...
      <tbody>
          ...
          <?php
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST") {
              ...
              if ($pencarian != '') {
                $sql = "SELECT id, answer, question FROM faq WHERE answer LIKE '%$pencarian%' OR question LIKE '%$pencarian%' ORDER BY id";
                $query = $sql;
                $queryJml = $sql;
              } else {
                $query = "SELECT id, answer, question FROM faq ORDER BY id LIMIT $posisi, $batas ";
                $queryJml = "SELECT id, answer, question FROM faq ORDER BY id";
                $no = $posisi + 1;
              }
            } else {
              $query = "SELECT id, answer, question FROM faq ORDER BY id LIMIT $posisi, $batas ";
                    ...
                    { 
                      ...
                      while($data = mysqli_fetch_array($querydata)){  
                          echo '<tr>';
                                                ...
                                                echo '<td><a href=index.php?halaman=edit_faq&&id='.$data['id'].'><span class="glyphicon glyphicon-edit"></a></td>';
                                                echo '<td><a href=config/hapus_faq.php?id='.$data['id'].'><span class="glyphicon glyphicon-remove-sign"></span></a></td>';
                                                echo '</tr>';
                        $no++;  
                      }
                    }
     ...
     if($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        <div style="float:right;">
          <ul class="pagination pagination-sm" style="margin: 0">
            ...
            for ($i=1; $i <= $jml_hal; $i++) {
              if ($i != $hal) {
                echo "<li><a href=\"index.php?halaman=data_faq&&hal=$i\">$i</a></li>";
              } else {
